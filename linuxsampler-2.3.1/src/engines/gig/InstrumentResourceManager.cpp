...
namespace LinuxSampler { namespace gig {
    ...
    InstrumentResourceManager::instrument_info_t InstrumentResourceManager::GetInstrumentInfo(instrument_id_t ID) throw (InstrumentManagerException) {
        ...
        try {
            ...
            size_t iRgn = 0;
            for (::gig::Region* pRegion = pInstrument->GetRegionAt(iRgn);
                 pRegion;       pRegion = pInstrument->GetRegionAt(++iRgn))
            {
            ...
            if (loaded) { // retrieve keyswitching only if the instrument is fully loaded.
                ...
                for (::gig::Region* pRegion = pInstrument->GetRegionAt(iRgn);
                     ...
                     pRegion = pInstrument->GetRegionAt(++iRgn))
                {
    ...
    InstrumentEditor* InstrumentResourceManager::LaunchInstrumentEditor(LinuxSampler::EngineChannel* pEngineChannel, instrument_id_t ID, void* pUserData) throw (InstrumentManagerException) {
        ...
        if (!vEditors.size()) {
            if (InstrumentEditorFactory::AvailableEditors().empty()) {
                fprintf(stderr,
                ...
                throw InstrumentManagerException(
                    "There is not any instrument editor installed and registered to the sampler"
                );
            }
            fprintf(stderr,
                "ERROR: Did not find a matching .gig editor for instrument "
                "('%s', %d) having data structure ('%s','%s'); installed "
                "and registered editor candidates were: %s\n"
                "[Cause: Make sure a .gig instrument editor (e.g. Gigedit) "
                "is installed to the sampler's plugin dir (%s) and that the "
                "installed editor version is binary compatible to this "
                "sampler's version (i.e. both must use the same libgig "
                "version).]\n",
                ID.FileName.c_str(), ID.Index,
                sDataType.c_str(), sDataVersion.c_str(),
                InstrumentEditorFactory::AvailableEditorsAsString().c_str(),
                InstrumentEditorFactory::PluginDirsAsString().c_str()
            );
            throw InstrumentManagerException(
                "There is no instrument editor installed and registered to "
                "the sampler that would be capable to handle this .gig instrument"
            );
        }
    ...
    ::gig::Instrument* InstrumentResourceManager::Create(instrument_id_t Key, InstrumentConsumer* pConsumer, void*& pArg) {
        ...
        pGig->GetSample(0); // just to force complete instrument loading
        dmsg(1,("OK\n"));
        ...
        size_t iRegion = 0;
        for (::gig::Region* pRgn = pInstrument->GetRegionAt(iRegion); pRgn;
                            pRgn = pInstrument->GetRegionAt(++iRegion))
        {
    ...
    std::set<EngineChannel*> InstrumentResourceManager::GetEngineChannelsUsingScriptSourceCode(const String& code, bool bLock) {
        ...
        std::set<InstrumentScriptConsumer*> consumers = scripts.ConsumersOf({
            ...
            .engineChannel = NULL, // just required for GCC
            .wildcardPatchVars = true,
            .wildcardEngineChannel = true
        });
    ...
    bool InstrumentResourceManager::SampleReferencedByInstrument(::gig::Sample* pSample, ::gig::Instrument* pInstrument) {
        size_t iRgn = 0;
        for (
            ::gig::Region* pRegion = pInstrument->GetRegionAt(iRgn);
            pRegion;       pRegion = pInstrument->GetRegionAt(++iRgn)
        ) {
            for (
    ...
    void InstrumentResourceManager::GigResourceManager::Destroy(::gig::File* pResource, void* pArg) {
        ...
        ssize_t iInstr = pResource->CountInstruments();
        for (--iInstr; iInstr >= 0; --iInstr) {
            ::gig::Instrument* instrument = pResource->GetInstrument(iInstr);
            bool deleteInstrument = true;
            ssize_t iRgn = instrument->CountRegions();
            for (--iRgn; iRgn >= 0; --iRgn) {
                ::gig::Region* region = instrument->GetRegionAt(iRgn);
        ...
        if (deleteFile) {
            ...
            ssize_t i = pResource->CountSamples();
            for (--i; i >= 0; --i) {
                ::gig::Sample* sample = pResource->GetSample(i);
